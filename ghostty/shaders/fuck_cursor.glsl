...
vec2 getRectangleCenterV2(vec4 rectangle) {
    // rectangle.xy 是矩形的左上角坐标
    // rectangle.z 是宽度
    // rectangle.w 是高度
    ...
    // Y 坐标：左上角 Y 加上高度的一半 (因为 Y 轴向下为正)
    float centerY = rectangle.y ; // <-- 关键修改这里
...
const float DURATION = 0.25; // 动画持续时间，单位秒
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec2 centerCP = getRectangleCenterV2(previousCursor);
    ...
    if (isCursorActuallyMoving) { // 只有在光标真正移动时才计算实际宽度
        ...
        if (abs(movementVector.x) > abs(movementVector.y)) {
            ...
            // 垂直移动为主：底边宽度基于光标宽度
            maxTrailHalfWidth = currentCursor.z * 0.4; // 可以根据需求调整此处的乘数
        }
        // 根据移动速度进一步调整宽度
        maxTrailHalfWidth = mix(maxTrailHalfWidth * 0.5, maxTrailHalfWidth * 0.95, taperFactor);
    }
    ...
    // trailOpacity *= (1.0 - easedProgress); // 随动画进度线性衰减
